...
df = pd.read_csv(r"C:\Users\basuk\Downloads\bassu\card_transdata.csv")
...
# Split the data
print("Splitting data into training and test sets (70% train, 30% test)...")
...
lr_model = GridSearchCV(LogisticRegression(max_iter=1000, random_state=42, solver='liblinear'), 
                       param_grid_lr, cv=3, scoring='f1', n_jobs=-1, verbose=2)
...
lr_accuracy, lr_preds = evaluate_model(lr_model.best_estimator_, X_test_scaled, y_test, "Logistic Regression")
...
dt_model = GridSearchCV(DecisionTreeClassifier(random_state=42), 
                       param_grid_dt, cv=3, scoring='f1', n_jobs=-1, verbose=2)
dt_model.fit(X_train_scaled, y_train)
train_time = time.time() - start_time
print(f"\nDecision Tree training completed in {train_time:.2f} seconds")
print(f"Best parameters: {dt_model.best_params_}")
dt_accuracy, dt_preds = evaluate_model(dt_model.best_estimator_, X_test_scaled, y_test, "Decision Tree")
...
sgd_model = GridSearchCV(SGDClassifier(loss='hinge', penalty='l2', random_state=42), 
                        param_grid_sgd, cv=3, scoring='f1', n_jobs=-1, verbose=2)
sgd_model.fit(X_train_scaled, y_train)
train_time = time.time() - start_time
print(f"\nSGD Classifier training completed in {train_time:.2f} seconds")
print(f"Best parameters: {sgd_model.best_params_}")
sgd_accuracy, sgd_preds = evaluate_model(sgd_model.best_estimator_, X_test_scaled, y_test, "SGD Classifier")
...
rf_model = GridSearchCV(RandomForestClassifier(random_state=42), 
                       param_grid_rf, cv=3, scoring='f1', n_jobs=-1, verbose=2)
rf_model.fit(X_train_scaled, y_train)
train_time = time.time() - start_time
print(f"\nRandom Forest training completed in {train_time:.2f} seconds")
print(f"Best parameters: {rf_model.best_params_}")
rf_accuracy, rf_preds = evaluate_model(rf_model.best_estimator_, X_test_scaled, y_test, "Random Forest")
